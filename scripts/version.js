...
const files_to_change = [
    ...
    "luarocks/allegro5_lua-scm-1.rockspec",
];
    ...
    next => {
        eachOfLimit(files_to_change, os.cpus().length, (filename, i, next) => {
            updateContent(sysPath.join(workspaceRoot, filename), oldContent => {
                return oldContent
                    .replace(new RegExp("(/lua-allegro5/releases/download/v|/lua-allegro5/tree/v)[0-9]+\\.[0-9]+\\.[0-9]+", "g"), `$1${ version }`)
                    .replace(new RegExp("(--branch v)[0-9]+\\.[0-9]+\\.[0-9]+( https://github.com/smbape/lua-allegro5)", "g"), `$1${ version }$2`);
            }, (err, hasChanged) => {
                next(err);
            });
        }, next);
