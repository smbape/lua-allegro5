...
function new_version.command(args)
   ...
   function persist.load_into_table(filename, tbl)
      ...
      local install_lib = {
         install_libdir .. "/allegro5_lua" .. shared_library_suffix,
      }
      recursive_include("allegro5_lua", install_libdir, install_lib)
      out_rs.build.install.lib = install_lib

      ---@type table<integer|string, string>
      local install_lua = {}
      recursive_include("allegro5_lua", install_sharedir, install_lua)
