...
# Update checks:
#   - check changes in CMakeList.txt *.cmake :
#       - check dependencies to update from vcpkg.json
#       - check new add_library to add ${ALLEGRO_MODULE_TYPE}
#   - check changes in examples
#   - check changes in include/allegro5/opengl/GLext/gl_ext_api.h include/allegro5/opengl/GLext/gl_ext_defs.h to update ALLEGRO_OPENGL_LAZY
...
# ================================
bash -c 'source scripts/tasks.sh && test_prepublished_source_debian test-source-ubuntu-26.04 ubuntu:26.04 -- test'
...
# generate doctoc
# ================================
cp -f out/prepublish/build/allegro5_lua/docs/docs.md ./docs/ && \
cp -f out/prepublish/build/allegro5_lua/generator/ids.json ./generator/ && \
