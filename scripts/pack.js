...
const Allegro5_NAME_VERSION = "allegro5-5.2.11.3";
const Allegro5_VERSION = Allegro5_NAME_VERSION.slice("allegro5-".length);
...
const getTasksSrcRock = srcRockSpec => {
    return [
        next => {
...
const getTasksBinaryRock = srcRockSpec => {
    return [
        next => {
        ...
        (_stdout, _stderr, next) => {
            ...
            const binary = `${ Allegro5_VERSION }${ target }${ ver }-${ distVersion }`;
            const binaryRockSpec = `${ srcRockSpec.slice(0, -`${ Allegro5_VERSION }-${ distVersion }.rockspec`.length) }${ binary }.rockspec`;
...
waterfall([
    ...
    (performed, next) => {
        spawnExec(luarocks, ["new_version", "--tag", `v${ version }`, options.rockspec, `${ Allegro5_VERSION }-${ distVersion }`], {
            stdio: "tee",
            cwd: workspaceRoot
        }, next);
    },
    ...
    (_stdout, _stderr, next) => {
        const srcRockSpec = _stdout.toString().trim().slice("Wrote ".length);
        const tasks = [];

        if (options.source) {
            tasks.push(...getTasksSrcRock(srcRockSpec));
        }

        if (options.binary) {
            tasks.push(...getTasksBinaryRock(srcRockSpec));
        }
