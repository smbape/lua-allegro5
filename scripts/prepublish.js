...
const prepublish = (target, version, options, next) => {
    ...
    const originalRockSpec = sysPath.join(projectRoot, "luarocks", "allegro5_lua-scm-1.rockspec");
    const scmRockSpec = name !== "allegro5_lua" ? sysPath.join(projectRoot, `${ name }-scm-1.rockspec`) : originalRockSpec;
    ...
    waterfall([
        ...
        (exists, next) => {
            ...
            if (name !== "allegro5_lua" || !keepSource) {
                tasks.push(async next => {
                    ...
                    if (name !== "allegro5_lua") {
                        rockspec = rockspec.replaceAll("allegro5_lua", name);
                    }
