-- https://github.com/lunarmodules/busted/blob/v2.2.0/busted/outputHandlers/gtest.lua
...
local io_write = function(format, ...)
    io.write(string_format(format, ...))
end
...
return function(options)
    local common, allegro, initialized

    for _, name in pairs({
        "outputHandlers.allegro.gtest",
        "examples.outputHandlers.allegro.gtest",
    }) do
        local module = package.searchpath(name, package.path)
        if module ~= nil then
            common = require(name:gsub("outputHandlers%.allegro%.gtest", "common"))

            local allegro5_lua = require("allegro5_lua")
            allegro = allegro5_lua.allegro5

            if not allegro.al_is_system_installed() then
                initialized = allegro.al_init()
                luassert.is_true(initialized, "Could not init Allegro.\n")
            end

            if common.env.ALLEGRO_POPUP_EXAMPLES then
                isatty = false
            end

            common.open_log()

            io_write = common.log_printf
        end
    end
    ...
    local pendingDescription = function(pending)
        ...
        if type(pending.message) == 'string' then
            str = str .. pending.message .. '\n'
        elseif pending.message ~= nil then
            str = str .. pretty.write(pending.message) .. '\n'
        end
    ...
    local failureDescription = function(failure)
        local str = failure.randomseed and ('Random seed: ' .. failure.randomseed .. '\n') or ''
        if type(failure.message) == 'string' then
            str = str .. failure.message
        elseif failure.message == nil then
            str = str .. 'Nil error'
        else
            str = str .. pretty.write(failure.message)
        end

        str = str .. '\n'

        if options.verbose and failure.trace and failure.trace.traceback then
            str = str .. failure.trace.traceback .. '\n'
        end
    ...
    local getSummary = function(status, count)
        ...
        if count > 0 and footer then
            local tests = (count == 1 and 'TEST' or 'TESTS')
            local errors = (count == 1 and 'ERROR' or 'ERRORS')
            str = string_format(footer, count, status == 'error' and errors or tests)
        end
        return str
    end
    ...
    local getSummaryString = function()
        ...
        local str = string_format(successStatus, successCount, tests)

        str = str .. getTestList('skipped', skippedCount, handler.pendings, pendingDescription)
        str = str .. getTestList('failure', failureCount, handler.failures, failureDescription)
        str = str .. getTestList('error', errorCount, handler.errors, failureDescription)

        str = str .. ((skippedCount + failureCount + errorCount) > 0 and '\n' or '')
        str = str .. getSummary('skipped', skippedCount)
        str = str .. getSummary('failure', failureCount)
        str = str .. getSummary('error', errorCount)
    ...
    handler.suiteStart = function(suite, count, total, randomseed)
        if total > 1 then
            io_write(repeatSuiteString, count, total)
        end
        if randomseed then
            io_write(randomizeString, randomseed)
        end
    ...
    handler.suiteEnd = function(suite, count, total)
        ...
        io_write(suiteEndString, testCount, tests, fileCount, files, elapsedTime_ms)
    ...
    handler.fileStart = function(file)
        fileTestCount = 0
        io_write(fileStartString, file.name)
        io_flush()
        return nil, true
    end
    ...
    handler.fileEnd = function(file)
        ...
        io_write(fileEndString, fileTestCount, tests, file.name, elapsedTime_ms)
        io_flush()
        return nil, true
    end

    handler.testStart = function(element, parent)
        io_write(runString, getFullName(element))
        io_flush()
    ...
    handler.testEnd = function(element, parent, status, debug)
        ...
        io_write(str, getFullName(element), elapsedTime_ms)
        io_flush()
