...
--[[
Sources:
    https://github.com/liballeg/allegro5/blob/5.2.11.3/examples/ex_clipboard.c
--]]
...
local function main(argv)
    ...
    while not done do
        ...
        if redraw and allegro5.al_is_event_queue_empty(queue) then
            if text then
                allegro5.al_free(text)
            end
            if allegro5.al_clipboard_has_text(display) then
                text = allegro5.al_get_clipboard_text(display)
            else
                text = nil
            end
            ...
            if text then
                allegro5.al_draw_text(font, allegro5.al_map_rgba_f(1, 1, 1, 1.0), 0, 0, 0, text)
            else
                allegro5.al_draw_text(font, allegro5.al_map_rgba_f(1, 0, 0, 1.0), 0, 0, 0,
        ...
        if event.type == allegro5.ALLEGRO_EVENT_KEY_DOWN then
            ...
            elseif event.keyboard.keycode == allegro5.ALLEGRO_KEY_SPACE then
                allegro5.al_set_clipboard_text(display, "Copied from Allegro!")
            end
