...
local EDOM = allegro5_lua.C.EDOM
local ERANGE = allegro5_lua.C.ERANGE

-- c functions as pure lua functions for faster execution
local function ALLEGRO_EVENT_TYPE_IS_USER(t)
    return t >= 512
end
allegro.ALLEGRO_EVENT_TYPE_IS_USER = ALLEGRO_EVENT_TYPE_IS_USER

local function ALLEGRO_GET_EVENT_TYPE(a, b, c, d)
    if type(a) == "string" then a = string.byte(a) end
    if type(b) == "string" then b = string.byte(b) end
    if type(c) == "string" then c = string.byte(c) end
    if type(d) == "string" then d = string.byte(d) end
    return bit.bor(bit.lshift(a, 24), bit.bor(bit.lshift(b, 16), bit.bor(bit.lshift(c, 8), d)))
end
allegro.ALLEGRO_GET_EVENT_TYPE = ALLEGRO_GET_EVENT_TYPE

local function ALLEGRO_USECS_TO_SECS(x)
    return x / 1000000.0
end
allegro.ALLEGRO_USECS_TO_SECS = ALLEGRO_USECS_TO_SECS

local function ALLEGRO_MSECS_TO_SECS(x)
    return x / 1000.0
end
allegro.ALLEGRO_MSECS_TO_SECS = ALLEGRO_MSECS_TO_SECS

local function ALLEGRO_BPS_TO_SECS(x)
    return 1.0 / x
end
allegro.ALLEGRO_BPS_TO_SECS = ALLEGRO_BPS_TO_SECS

local function ALLEGRO_BPM_TO_SECS(x)
    return 60.0 / x
end
allegro.ALLEGRO_BPM_TO_SECS = ALLEGRO_BPM_TO_SECS


if type(clib) ~= "nil" then
    local function _AL_MAP_RGBA(_color, _r, _g, _b, _a)
        _color.r = _r / 255
        _color.g = _g / 255
        _color.b = _b / 255
        _color.a = _a / 255
    ...
    --[[ Function: al_map_rgba
     --]]
    local function al_map_rgba(r, g, b, a)
        local color = allegro.ALLEGRO_COLOR()
        _AL_MAP_RGBA(color, r, g, b, a)
        return color
    end
    allegro.al_map_rgba = al_map_rgba


    --[[ Function: al_premul_rgba
     --]]
    local function al_premul_rgba(r, g, b, a)
        local color = allegro.ALLEGRO_COLOR()
        _AL_MAP_RGBA(color, r * a / 255, g * a / 255, b * a / 255, a)
        return color
    end
    allegro.al_premul_rgba = al_premul_rgba


    --[[ Function: al_map_rgb
     --]]
    local function al_map_rgb(r, g, b)
        return allegro.al_map_rgba(r, g, b, 255)
    end
    allegro.al_map_rgb = al_map_rgb


    --[[ Function: al_map_rgba_f
     --]]
    local function al_map_rgba_f(r, g, b, a)
        local color = allegro.ALLEGRO_COLOR()
        color.r = r
        color.g = g
        color.b = b
    ...
    allegro.al_map_rgba_f = al_map_rgba_f


    --[[ Function: al_premul_rgba_f
     --]]
    local function al_premul_rgba_f(r, g, b, a)
        local color = allegro.ALLEGRO_COLOR()
        color.r = r * a
        color.g = g * a
        color.b = b * a
    ...
    allegro.al_premul_rgba_f = al_premul_rgba_f


    --[[ Function: al_map_rgb_f
     --]]
    local function al_map_rgb_f(r, g, b)
        return allegro.al_map_rgba_f(r, g, b, 1.0)
    end
    allegro.al_map_rgb_f = al_map_rgb_f
    ...
    --[[ Function: al_unmap_rgba
     --]]
    local function al_unmap_rgba(color)
        return color.r * 255.0, color.g * 255.0, color.b * 255.0, color.a * 255.0
    end
    allegro.al_unmap_rgba = al_unmap_rgba


    --[[ Function: al_unmap_rgb
     --]]
    local function al_unmap_rgb(color)
        return color.r * 255.0, color.g * 255.0, color.b * 255.0
    end
    allegro.al_unmap_rgb = al_unmap_rgb


    --[[ Function: al_unmap_rgba_f
     --]]
    local function al_unmap_rgba_f(color)
        return color.r, color.g, color.b, color.a
    end
    allegro.al_unmap_rgba_f = al_unmap_rgba_f


    --[[ Function: al_unmap_rgb_f
     --]]
    local function al_unmap_rgb_f(color)
        return color.r, color.g, color.b
    end
    allegro.al_unmap_rgb_f = al_unmap_rgb_f
end
...
local function al_ftofix(x)
    if x > 32767.0 then
        allegro.al_set_errno(ERANGE)
        return 0x7FFFFFFF
    end
    ...
    return math.floor(x * 65536.0 + ((function() if x < 0 then return -0.5 else return 0.5 end end)()))
end
allegro.al_ftofix = al_ftofix


local function al_fixtof(x)
    return x / 65536.0
end
allegro.al_fixtof = al_fixtof


local function al_fixadd(x, y)
    local result = x + y
...
allegro.al_fixadd = al_fixadd


local function al_fixsub(x, y)
    local result = x - y
...
--     if lres > 0x7FFFFFFF0000 then
--         allegro.al_set_errno(ERANGE)
--         return 0x7FFFFFFF
--     elseif lres < -0x7FFFFFFF0000 then
--         allegro.al_set_errno(ERANGE)
--         return 0x80000000
--     else
--         return bit.arshift(lres, 16)
--     end
-- end
-- #endif       /* al_fixmul() C implementations */
allegro.al_fixmul = al_fixmul


-- #if defined ALLEGRO_CFG_NO_FPU
-- local function al_fixdiv(x, y)
--    local lres = x
--    if y == 0 then
--       allegro.al_set_errno(ERANGE)
--       return (function() if (x < 0) then return  -0x7FFFFFFF else return  0x7FFFFFFF end end)()
--    end
--    lres = bit.lshift(lres, 16)
--    lres = math.floor(lres / y)
--    if lres > 0x7FFFFFFF then
--       allegro.al_set_errno(ERANGE)
--       return 0x7FFFFFFF
--    elseif lres < -0x7FFFFFFF then
--       allegro.al_set_errno(ERANGE)
--       return 0x80000000
--    else
...
local function al_fixfloor(x)
    --[[ (x >> 16) is not portable --]]
    if x >= 0 then
        return bit.arshift(x, 16)
    else
        return bit.bnot(bit.arshift(bit.bnot(x), 16))
    end
end
allegro.al_fixfloor = al_fixfloor


local function al_fixceil(x)
    if x > 0x7FFF0000 then
        allegro.al_set_errno(ERANGE)
        return 0x7FFF
    end
...
allegro.al_fixceil = al_fixceil


local function al_itofix(x)
    return bit.lshift(x, 16)
end
allegro.al_itofix = al_itofix


local function al_fixtoi(x)
    return al_fixfloor(x) + (bit.arshift((bit.band(x, 0x8000)), 15))
end
allegro.al_fixtoi = al_fixtoi


local function al_fixcos(x)
    return _al_fix_cos_tbl[bit.band((bit.arshift((x + 0x4000), 15)), 0x1FF) + 1 --[[ INDEX_BASE --]]]
end
allegro.al_fixcos = al_fixcos


local function al_fixsin(x)
    return _al_fix_cos_tbl[bit.band((bit.arshift((x - 0x400000 + 0x4000), 15)), 0x1FF) + 1 --[[ INDEX_BASE --]]]
end
allegro.al_fixsin = al_fixsin


local function al_fixtan(x)
    return _al_fix_tan_tbl[bit.band((bit.arshift((x + 0x4000), 15)), 0xFF) + 1 --[[ INDEX_BASE --]]]
end
allegro.al_fixtan = al_fixtan


local function al_fixacos(x)
    if (x < -65536) or (x > 65536) then
        allegro.al_set_errno(EDOM)
        return 0
    end

    return _al_fix_acos_tbl[bit.arshift((x + 65536 + 127), 8) + 1 --[[ INDEX_BASE --]]]
end
allegro.al_fixacos = al_fixacos


local function al_fixasin(x)
    if (x < -65536) or (x > 65536) then
        allegro.al_set_errno(EDOM)
        return 0
    end

    return 0x00400000 - _al_fix_acos_tbl[bit.arshift((x + 65536 + 127), 8) + 1 --[[ INDEX_BASE --]]]
end
allegro.al_fixasin = al_fixasin
